...
import { MarkResizable, SetOptional, Lazy, GetOptional, CreateFuncWithArgs, DeleteEmptyFields, Create as Create_2, OnLoad } from "./WebSharper.Core.JavaScript/Runtime.js"
...
function Game(){
  ...
  const S=Doc.Convert((_3) => {
    const this_7=new ProviderBuilder("New_1");
    const this_8=(this_7.h.push(new Text("stockname", _3[0])),this_7);
    const b_1=(this_8.h.push(new Text("stockamount", String(_3[1]))),this_8);
    const p_1=CompleteHoles(b_1.k, b_1.h, []);
    const i_1=new TemplateInstance(p_1[1], stockitem(p_1[0]));
    let _4=(b_1.i=i_1,i_1);
    return _4.Doc;
  }, stocksList().v);
  const M=(c=money().Get(),String(c));
  const S_1=Doc.Convert((_3) => {
    let c_1;
    const name=_3[0];
    const P=(c_1=(lastDayStocksPrice().Find((t_3) => t_3[0]==name))[1],String(c_1));
    const this_7=new ProviderBuilder("New_1");
    const this_8=(this_7.h.push(new Text("stockname", name)),this_7);
    const this_9=(this_8.h.push(new Text("stockprice", String(_3[1]))),this_8);
    const b_1=(this_9.h.push(new Text("pricepercentage", P)),this_9);
    const p_1=CompleteHoles(b_1.k, b_1.h, []);
    const i_1=new TemplateInstance(p_1[1], stockpriceitem(p_1[0]));
    let _4=(b_1.i=i_1,i_1);
    return _4.Doc;
  }, stocksPrice().v);
  const this_1=new ProviderBuilder("New_1");
  const this_2=(this_1.h.push(new Elt("stockpricecontainer", S_1)),this_1);
  const this_3=(this_2.h.push(new VarStr("moneyowned", _c_1.Create_1(M))),this_2);
  const t=(this_3.h.push(new Elt("stockcontainer", S)),this_3);
  const this_4=(t.h.push(EventQ2(t.k, "nextdaybutton", () => t.i, () => {
    const posOrNeg=rnd.Next(0, 2)*2-1;
    const arrayValues=[(stocksPrice().Find((t_3) => t_3[0]=="NASDAQ"))[1], (stocksPrice().Find((t_3) => t_3[0]=="RUSSEL"))[1], (stocksPrice().Find((t_3) => t_3[0]=="IBEX"))[1], (stocksPrice().Find((t_3) => t_3[0]=="NIKKEI"))[1]];
    lastDayStocksPrice().Clear();
    lastDayStocksPrice().AppendMany(stocksPrice());
    stocksPrice().Clear();
    stocksPrice().AppendMany([["NASDAQ", get(arrayValues, 0)+posOrNeg*rnd.Next(0, 1000)], ["RUSSEL", get(arrayValues, 1)+posOrNeg*rnd.Next(0, 300)], ["IBEX", get(arrayValues, 2)+posOrNeg*rnd.Next(0, 600)], ["NIKKEI", get(arrayValues, 3)+posOrNeg*rnd.Next(0, 1500)]]);
  })),t);
  const this_5=(this_4.h.push(new VarStr("selectedstock", selectedStock())),this_4);
  const this_6=(this_5.h.push(new VarIntUnchecked("inputbuy", inputBuy())),this_5);
  const t_1=(this_6.h.push(new VarIntUnchecked("inputsell", inputSell())),this_6);
  const t_2=(t_1.h.push(EventQ2(t_1.k, "buybutton", () => t_1.i, (t_3) => {
    let c_1;
    const amount=(stocksList().Find((t_4) => t_4[0]==selectedStock().Get()))[1];
    const price=(stocksPrice().Find((t_4) => t_4[0]==selectedStock().Get()))[1];
    const a=money().Get();
    const b_1=inputBuy().Get();
    if(b_1>=0){
      const _3=b_1*price;
      if(a>=_3){
        money().Set(a-_3);
        stocksList().Append([selectedStock().Get(), amount+inputBuy().Get()]);
      }
      else money().Set(a);
    }
    else money().Set(a);
    TemplateHole.Value(t_3.Vars.Hole("moneyowned")).Set((c_1=money().Get(),String(c_1)));
    inputBuy().Set(0);
  })),t_1);
  const b=(t_2.h.push(EventQ2(t_2.k, "sellbutton", () => t_2.i, (t_3) => {
    let c_1;
    const amount=(stocksList().Find((t_4) => t_4[0]==selectedStock().Get()))[1];
    const price=(stocksPrice().Find((t_4) => t_4[0]==selectedStock().Get()))[1];
    const b_1=inputSell().Get();
    if(b_1>=0)amount>=b_1?(money().Set(money().Get()+b_1*price),stocksList().Append([selectedStock().Get(), amount-inputSell().Get()])):stocksList().Append([selectedStock().Get(), amount]);
    else stocksList().Append([selectedStock().Get(), amount]);
    TemplateHole.Value(t_3.Vars.Hole("moneyowned")).Set((c_1=money().Get(),String(c_1)));
    inputSell().Set(0);
  })),t_2);
  const p=CompleteHoles(b.k, b.h, [["moneyowned", 0, null], ["selectedstock", 0, null], ["inputbuy", 1, null], ["inputsell", 1, null]]);
  const i=new TemplateInstance(p[1], game(p[0]));
  let _1=(b.i=i,i);
  const _2=_1.Doc;
  LoadLocalTemplates("");
  Doc.RunById("game", _2);
...
function selectedStock(){
  return _c.selectedStock;
}
function stocksPrice(){
  return _c.stocksPrice;
}
function lastDayStocksPrice(){
  return _c.lastDayStocksPrice;
}
function money(){
  return _c.money;
}
function stocksList(){
  return _c.stocksList;
}
class Object_1 {
  Equals(obj){
    return this===obj;
...
function stockpriceitem(h){
  LoadLocalTemplates("index");
  return h?NamedTemplate("index", Some("stockpriceitem"), h):void 0;
}
function stockitem(h){
  LoadLocalTemplates("index");
  return h?NamedTemplate("index", Some("stockitem"), h):void 0;
}
function game(h){
  LoadLocalTemplates("index");
  return h?NamedTemplate("index", Some("game"), h):void 0;
}
...
class VarIntUnchecked extends TemplateHole {
  name;
  fillWith;
  get ValueObj(){
    return this.Value;
...
let _c=Lazy((_i) => class $StartupCode_Client {
  ...
  static lastDayStocksPrice;
  static stocksPrice;
  static stocksList;
  static selectedStock;
  static money;
  static inputSell;
  static inputBuy;
  static {
    this.inputBuy=_c_1.Create_1(0);
    this.inputSell=_c_1.Create_1(0);
    this.money=_c_1.Create_1(100000);
    this.selectedStock=_c_1.Create_1("NASDAQ");
    this.stocksList=Create((t) => t[0], ofArray([["NASDAQ", 0], ["RUSSEL", 0], ["IBEX", 0], ["NIKKEI", 0]]));
    this.stocksPrice=Create((t) => t[0], ofArray([["NASDAQ", 16300], ["RUSSEL", 1890], ["IBEX", 13000], ["NIKKEI", 34800]]));
    this.lastDayStocksPrice=Create((t) => t[0], ofArray([["NASDAQ", 16300], ["RUSSEL", 1890], ["IBEX", 13000], ["NIKKEI", 34800]]));
  }
});
